...
class WaterQualityInput(BaseModel):
    ...
    conductivity_µmhocmmin: Optional[float] = Field(None, alias="Conductivity (µmho/cm)(Min)")
    conductivity_µmhocmmax: Optional[float] = Field(None, alias="Conductivity (µmho/cm)(Max)")
...
try:
    ...
    PREPROCESSOR_ARTIFACT = MODEL_PIPELINE = LABEL_ENCODER = None
...
async def check_artifacts():
    if any(art is None for art in [PREPROCESSOR_ARTIFACT, MODEL_PIPELINE, LABEL_ENCODER]):
...
async def predict_wqi(data: WaterQualityInput):
    ...
    try:
        ...
        if hasattr(MODEL_PIPELINE, "predict_proba"):
            proba = MODEL_PIPELINE.predict_proba(processed_df)
            confidence = np.max(proba, axis=1)[0] * 100
        else:
            confidence = 100.0
        
        logger.info(f"Prediction: {predicted_category} with {confidence:.2f}% confidence.")
